...
public class LinkedListTabulatedFunction implements TabulatedFunction, Serializable, Cloneable {
    ...
    */
    ...
    public LinkedListTabulatedFunction(FunctionPoint[] points){
        ...
        head = new FunctionNode();
        FunctionNode currentElement = new FunctionNode();
        ...
        for (int i = 1; i < points.length; i++, currentIndex++, currentElement = currentElement.next){
            currentElement.next = new FunctionNode();
            currentElement.next.prev = currentElement;
            currentElement.point = points[i];
            currentElement.prev = head;
        }
    ...
    public void printFunction(){
        currentIndex = 0;
        for (currentElement = head; currentElement != tail; currentElement = currentElement.next, ++currentIndex){
            System.out.println("Point " + (currentIndex + 1));
            System.out.println("x = " + currentElement.point.getX());
            System.out.println("y = " + currentElement.point.getY());
        }
        System.out.println("Point " + (currentIndex + 1));
    ...
    public FunctionPoint getPoint(int index){
        getNodeByIndex(index);
        return new FunctionPoint(currentElement.point);
    }
    ...
    public String toString(){
        ...
        currentIndex = 0;
        for (currentElement = head; currentElement != tail; currentElement = currentElement.next, currentIndex++){
            result.append(currentElement.point.toString());
            result.append(", ");
        }
        result.append(tail.point.toString());
        result.append("}");
    ...
    public boolean equals(Object o){
        ...
        if (o instanceof TabulatedFunction){
            if (o instanceof LinkedListTabulatedFunction){
                ...
                currentIndex = 0;
                for (currentElement = head; currentElement != tail.next; currentElement = currentElement.next, currentIndex++){
                    if (func.getPoint(currentIndex).getX() != this.getPoint(currentIndex).getX() || func.getPoint(currentIndex).getY() != this.getPoint(currentIndex).getY()){
                        return false;
                    }
                }
                return true;
    ...
    public Object clone(){
        ...
        for (currentElement = head; currentElement != tail; currentElement = currentElement.next, currentIndex++){
            functionPoints[currentIndex] = (FunctionPoint) currentElement.point.clone();
        }
        functionPoints[currentIndex] = (FunctionPoint) tail.point.clone();
